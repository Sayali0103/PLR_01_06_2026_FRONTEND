...
const fieldClass = 'w-full rounded-[12px] border border-black/10 bg-white px-4 py-3 text-[14px] text-[#1a1208] outline-none transition-all duration-200 focus:border-[#FF7D00] focus:ring-4 focus:ring-[#FF7D00]/10'
  ...
  return (
    <label className={field.fullWidth ? 'md:col-span-2' : ''}>
      <span className="mb-2 block text-[12px] font-bold tracking-[1.3px] uppercase text-[#7b6c5e]">
        {field.label} <span className="text-[#FF7D00]">*</span>
      </span>
      {field.type === 'textarea' ? (
      ...
      <div className="max-w-[1120px] mx-auto grid lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-20 items-start">
        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
          ...
          {helperLink && (
            <Link
              to={helperLink.to}
              className="mt-6 inline-flex items-center gap-2 rounded-[12px] border border-[#FF7D00]/20 bg-white px-5 py-3 text-[13px] font-bold text-[#FF7D00] shadow-[0_8px_28px_rgba(26,18,8,0.05)]"
            >
              {helperLink.label}
          ...
          <div className="mt-10 rounded-2xl border border-[#FF7D00]/15 bg-[#fff8f0] p-5 text-[13px] leading-[1.8] text-[#6d5f51]">
            Prefer email? Write to <a href="mailto:contact@plrobotics.com" className="font-bold text-[#FF7D00]">contact@plrobotics.com</a>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.08 }} className="rounded-[24px] border border-black/5 bg-white p-6 sm:p-8 shadow-[0_18px_55px_rgba(26,18,8,0.08)]">
          {state.success ? (
            <div className="py-10 text-center">
              <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-[#FF7D00]/10 text-[26px] text-[#FF7D00]">✓</div>
              ...
              <Link to="/" className="inline-flex rounded-[10px] bg-[#FF7D00] px-6 py-3 text-[13px] font-bold text-white">Back to Home</Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit}>
              ...
              <button disabled={state.submitting} className="mt-7 inline-flex w-full items-center justify-center rounded-[12px] bg-[#FF7D00] px-7 py-[14px] text-[13px] font-bold text-white shadow-[0_5px_22px_rgba(255,125,0,0.3)] transition-opacity disabled:opacity-60">
